      ...
      for (i=0; i<resources.length; i++) {
        ...
        */
        if (resources[i].tags.indexOf($text) != -1 && resources[i].approved === true) {
          $("#resources").append("<div class=\"col-md-6 masonry-item\"> <div class=\"resource book\"> <button class=\"pull-right btn btn-sm btn-primary\"><i class=\"fa fa-plus\"></i></button> <h3><i class=\"fa fa-book\"></i>&nbsp;" + resources[i].name +"</h3> <p><em>"+resources[i].description+"</em></p> <p><a href="+ resources[i].url +"><i class=\"fa fa-external-link\"></i>&nbsp;"+ resources[i].name +"</a></p> </div> </div>")
        }
      }
    })
    for (i=0; i<resources.length; i++) {
